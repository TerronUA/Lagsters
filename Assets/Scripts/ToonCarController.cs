...
namespace Lagsters.Car
{
    ...
    public class ToonCarController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (m_CenterOfMass)
                centerOfMass = m_CenterOfMass.transform.localPosition;
            else
                centerOfMass = m_CentreOfMassOffset;

            m_WheelMeshLocalRotations = new Quaternion[4];
            for (int i = 0; i < 4; i++)
            {
                m_WheelMeshLocalRotations[i] = m_WheelMeshes[i].transform.rotation;
                m_WheelColliders[i].attachedRigidbody.centerOfMass = centerOfMass;
                //m_WheelColliders[i].ConfigureVehicleSubsteps();
            }
